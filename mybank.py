...
# my_cursor.execute("CREATE DATABASE IF NOT EXISTS mybank")

bank_db = mysql.connector.connect(
    host="b3gvbkopy8lcujyg35aw-mysql.services.clever-cloud.com",
    user="uhlv5dnwu2kuptal",
    password="1Fsg2hqs9QQ3AxEMGoPr",
    database="b3gvbkopy8lcujyg35aw"
)

my_cursor = bank_db.cursor()
#my_cursor.execute("DROP TABLE accounts")
my_cursor.execute("CREATE TABLE IF NOT EXISTS accounts (name varchar(255), accno varchar(20), dob varchar(20), addr varchar(255), zip varchar(50), phone varchar(50), balance decimal(20,4), password varchar(255), id int AUTO_INCREMENT PRIMARY KEY)")
...
def myfont(s):
    return ("Lucida Console",s, font.BOLD)
...
#title_bar.configure(font=("Segoe Script",19,font.BOLD))
...
createBtn = Button(root, text="CREATE Account",bd=5, command=createAcc)
createBtn.grid(row=1, column=0,columnspan=2, padx=10, pady=10, ipadx=90)

depositBtn = Button(root, text="DEPOSIT Money",bd=5, command=deposit)
depositBtn.grid(row=2, column=0,columnspan=2, padx=12, pady=10, ipadx=92)

withdrawBtn = Button(root, text="WITHDRAW Money",bd=5, command=withdraw)
withdrawBtn.grid(row=3, column=0,columnspan=2, padx=7, pady=10, ipadx=84)

detailsBtn = Button(root, text="DISPLAY Account Details",bd=5, command=details)
detailsBtn.grid(row=4, column=0,columnspan=2, padx=10, pady=10, ipadx=65)

balanceBtn = Button(root, text="BALANCE Enquiry",bd=5, command=balanceEnq)
balanceBtn.grid(row=5, column=0,columnspan=2, padx=10, pady=10, ipadx=85)

updateBtn = Button(root, text="UPDATE Details",bd=5, command=updateAcc)
updateBtn.grid(row=6, column=0,columnspan=2, padx=10, pady=10, ipadx=93)

changePwdBtn = Button(root, text="CHANGE PASSWORD",bd=5, command=changePassword)
changePwdBtn.grid(row=7, column=0,columnspan=2, padx=10, pady=10, ipadx=78)

transferBtn = Button(root, text="TRANSFER Money",bd=5, command=transfer)
transferBtn.grid(row=8, column=0,columnspan=2, padx=10, pady=10, ipadx=87)

closeBtn = Button(root, text="CLOSE Account",bd=5, command=closeAcc)
closeBtn.grid(row=9, column=0,columnspan=2, padx=10, pady=10, ipadx=93)
